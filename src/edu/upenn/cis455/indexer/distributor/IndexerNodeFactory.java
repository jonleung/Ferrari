 ...
 *
 */
public class IndexerNodeFactory {
	Environment env;
...
//	InetSocketAddress boot;
	...
	IndexerNodeFactory(int port) {
		this(new Environment(), port);
	}	
	
	public IndexerNodeFactory(int port, InetSocketAddress bootAddress) {
		this(port);
//		boot = bootAddress;
		bootHandle = factory.getNodeHandle(bootAddress);
	}
	
	IndexerNodeFactory(Environment env, int port) {
		this.env = env;
		this.port = port;
	...
	public Node getNode() {
		try {
			...
			String nodeID = null;
			NodeIDAccess accessID = new NodeIDAccess(ReverseIndexCollectionNode.db.getEnv());
