...
public class ReverseIndexCollectionNode {
	...
	public static void main(String...args) {
		...
		if(args.length < 7 || args.length > 8) {
			System.out.println("Usage: ReverseIndexCollectionNode <local ip> <local port> <bootstrap ip> " +
					"<bootstrap port> <daemon port> <bekeleydb_path> <reverse_index_dir> [startChar-endChar]");
			System.exit(0);
		}
		...
		if (BOOT_NODE) {
			...
			try {
				WordStorageHandler handler = new WordStorageHandler(daemonPort);
				handler.start();
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static void startDistributer() {
		if(BOOT_NODE) {
			System.out.println("Starting distributer");
			ReverseIndexDistributer rid = new ReverseIndexDistributer();
			rid.distribute(dirname);
		}
	}
