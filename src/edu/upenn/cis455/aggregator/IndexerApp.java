	...
	public void deliver(Id id, Message message) { 
		...
		switch(recievedMsg.messageType) {
			case JOIN: 		if(node.getLocalNodeHandle() != recievedMsg.from) {
								...
								String reply = null;
								reply = AggregatorEntry.characters + "<>" + AggregatorEntry.localAddr + "<>" + AggregatorEntry.daemonPort;
			...
			case CHARS:		System.out.println("GOT CHARACTERS" + recievedMsg.alpha);
							...
							for(int i = temp[0].charAt(0); i<= temp[1].charAt(0) ; i++) {
								AggregatorEntry.charMapping.put((char) i, charNode);
								System.out.println("charMapping: " + (char)i);
							}
//							if(!AggregatorEntry.socketMapping.containsKey(temp[0].charAt(0))) {
//								Socket socket = null;
...
//								}
//								for(int i = temp[0].charAt(0); i<= temp[1].charAt(0) ; i++) {
//									AggregatorEntry.socketMapping.put((char) i, out);
